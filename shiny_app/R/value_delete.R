...
car_delete_module <- function(input, output, session, modal_title, car_to_delete, modal_trigger) {
  ...
  observeEvent(modal_trigger(), {
    ...
    req(session$userData$email == 'shivani.avera@gmail.com')
        ...
        div(
          ...
          h2(
            ...
            paste0(
              ...
              car_to_delete()$Gene," ID: ",car_to_delete()$EntryID,
              '"?'
            )
          )
        ),
  ...
  observeEvent(input$delete_button, {
    ...
    tryCatch({
      ...
      uid <- car_to_delete()$EntryID
      
      DBI::dbExecute(
        conn,
        "DELETE FROM knowTemp WHERE EntryID=$1",
        params = c(
          list(uid)
        )
      )
